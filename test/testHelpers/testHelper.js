...
th.status_is_file_not_found = (res)=>assert.equal(res.statusCode,404);
...
th.should_have_expiring_cookie_on = (res,name,value,time)=> {
  console.log(res.headers);
  let cookieText = res.headers['Set-Cookie'];
  assert.include(cookieText,`${name}=${value}; Max-Age=${time}`);
};
module.exports = th;
